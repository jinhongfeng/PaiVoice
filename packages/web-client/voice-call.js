...
export class VoiceCall {
  ...
  async _onMessage(data) {
    ...
    switch (msg.type) {
      ...
      case 'config': this.emit('config', msg); break;            // 设置面板：当前配置快照
      case 'config_error': this.emit('configError', msg); break; // 设置失败（模型不存在/音色切换失败）
      case 'profiles': this.emit('profiles', msg); break;        // API 配置档案列表已更新
      case 'profile_error': this.emit('profileError', msg); break;
      case 'profile_test_result': this.emit('profileTest', msg); break; // API 连接测试结果
